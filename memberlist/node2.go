...
type MyEventDelegate2 struct {
	nodeName string
}

func (d *MyEventDelegate2) NotifyJoin(node *memberlist.Node) {
	log.Printf("[%s] *** 通知: 节点加入了: %s (%s:%d)\n", d.nodeName, node.Name, node.Addr, node.Port)
}

func (d *MyEventDelegate2) NotifyLeave(node *memberlist.Node) {
	log.Printf("[%s] *** 通知: 节点离开了 (或被标记为故障): %s (%s:%d)\n", d.nodeName, node.Name, node.Addr, node.Port)
}

func (d *MyEventDelegate2) NotifyUpdate(node *memberlist.Node) {
	log.Printf("[%s] *** 通知: 节点元数据更新了: %s (元数据: %s)\n", d.nodeName, node.Name, string(node.Meta))
}

func main() {
	nodeName := "node-B"
	bindPort := 7947             // Node B 的监听端口
	...
	config.Events = &MyEventDelegate2{nodeName: nodeName}
